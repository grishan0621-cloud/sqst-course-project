...
## Типовые проблемы запуска (сравнительная часть, Semgrep)

1. **`Read timed out` на `--config=auto` / онлайн-профилях.** Публичный
   реестр semgrep.dev при загрузке правил часто отваливается по тайм-ауту
   (`HTTPSConnectionPool(host='semgrep.dev', port=443): Read timed out`).
   Не повторяйте запуск бесконечно — переходите на офлайн-набор правил:
   склонируйте `semgrep/semgrep-rules` и подключите каталог через bind
   mount, либо запеките правила в собственный Docker-образ. Оба рецепта —
   в методичке урока 15, раздел «Установка и запуск Semgrep».

2. **3–5 находок вместо 20+.** Признак того, что скан прошёл по
   урезанному онлайн-профилю (или оборвался на тайм-ауте): на этом же
   коде офлайн-набор `semgrep-rules/python` + `semgrep-rules/javascript`
   даёт в разы больше находок. Если сравнение инструментов внезапно
   показывает «Semgrep почти ничего не нашёл» — сначала проверьте, каким
   набором правил вы сканировали, и повторите скан офлайн-набором.
